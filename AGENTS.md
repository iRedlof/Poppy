# Poppy – Dev Notes for Agents

This file provides guidance for coding agents working in this repo.

## Project Overview

Poppy is a friendly macOS menu bar shortcut shelf. Users launch apps, run shell commands, open URLs, and manage developer workflows from a single popover triggered by ⌘⇧K.

The app, repo, Xcode project, scheme, bundle identifier, and Swift package now consistently use the Poppy/PoppyCore naming.
...
# Run tests (must be run from PoppyCore directory)
cd PoppyCore && swift test

# Open in Xcode (recommended for development)
xcodegen generate && open Poppy.xcodeproj
```
...
Releases are created by the GitHub Actions `Release` workflow. It signs and notarizes `Poppy.app`, creates a notarized DMG, generates Sparkle `appcast.xml`, and uploads the DMG plus appcast to GitHub Releases.
